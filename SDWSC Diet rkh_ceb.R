...
all = full_join(flash_all, dop) %>% 
  ...
  select(Project, SerialNumber, Year, Month, Date, Station, SampleID, Species, ForkLength, TotalBodyWeight, 'Acanthocyclops spp' :'Nippoleucon hinumensis')
...
sdwscdiet_long = pivot_longer(sdwsc_diet, 
  ...
  select(Project:SampleID, latitude, longitude, Region, Species, ForkLength, 
         TotalBodyWeight, DietCategory, DietbyNumber, SerialNumber) %>% 
...
sdwscdiet_wide = pivot_wider(sdwscdiet_long, id_cols = Project:TotalBodyWeight, 
...
# 
lengths = read_xlsx("data/Diet Prey Lengths to 2022.xlsx") %>% 
  merge(., stations, by= c("Project", "Station")) %>% 
  mutate(SerialNumber = str_remove(SerialNumber, "^0+")) %>%  #for some reason the serial numbers here have leading zeros, so need to remove so we can combine macro and meso files with matching serial numbers later. 
  #use str remove and the "^0+" says to remove the 0s at the start of the number. Would normally convert to numeric but have some characters in there that were alphnumeric
  mutate(SampleID = paste(Database, LogNumber, Project, Date, Station, SerialNumber, sep = " ")) #create sample ID
...
sdwsc_lengths = read_csv("shipchannel_lengths.csv")
...
lw_conversions = read_xlsx("data/Mesomicromacro Biomass conversions.xlsx", sheet = "Macro-zooplankton") %>% 
  filter(Preservative != "Formalin", Weight_type != "Dry") %>%  #filtering so its just wet and not formalin
...
macrobm = lw %>% 
  ...
  select(Year, Month, Region, SampleID, IBMR, Estimate, DietBiomass)
...
totmacrozeros = pivot_wider(totmacro, names_from = IBMR, values_from = totibmr, values_fill = 0) %>%
  pivot_longer(cols = c(amphipod, mysids), names_to = "IBMR", values_to = "totibmr")
...
diet_ibmr= allbiomass %>% 
  group_by(Year, Month, Region, SampleID, ForkLength, TotalBodyWeight, IBMR) %>% 
...
diet_ibmr0s = pivot_wider(diet_ibmr, names_from = IBMR, 
                          id_cols = c(SampleID, ForkLength, TotalBodyWeight, Month, Year, Region),
                             values_from = tot_samp, values_fill = 0) %>%
  pivot_longer(cols = c(acartela:mysids), names_to = "IBMR", values_to = "tot_samp") %>%
  group_by(SampleID) %>%
  mutate(propdiet = tot_samp/(sum(tot_samp))) %>%
  ungroup()


write.csv(diet_ibmr0s, file = "diet_ibmr.csv")

#it sounds like Will need it in wide format, rather than long

diet_ibmr_wide = pivot_wider(diet_ibmr0s, id_cols = c(SampleID, ForkLength, 
                                                      TotalBodyWeight, Month, Year, Region),
                             names_from = IBMR, values_from = propdiet)

write.csv(diet_ibmr_wide, "diet_ibmr_wide.csv")

diet_ibmr_wide2 = pivot_wider(diet_ibmr0s, id_cols = c(SampleID, ForkLength, 
                                                      TotalBodyWeight, Month, Year, Region),
                             names_from = IBMR, values_from = tot_samp)

#ugh, things didn't work right, but I think i can fix it later

write.csv(diet_ibmr_wide2, "diet_ibmr_mass.csv")
...
#################################################################
#add environmentals
wide2 = read_csv("diet_ibmr_wide_c.csv")
wide2x = wide2

wide2x[is.na(wide2)] =0


#ugh, no there were issues earlier. Let's try this again.
lastgroup <- function(string) {
...
test = lastgroup(wide2z$SampleID)

wide2z$serial =tail(str_split(wide2z$SampleID, " ")[[1]], n =1)

wide2z = group_by(wide2x, SampleID) %>%
  mutate(serial = lastgroup(SampleID), SampleID2 = str_remove(SampleID, serial),
         serial = str_remove(serial, "^0+")) 


wide2.1 = group_by(wide2z, SampleID2, serial, Region) %>%
  summarize(across(ForkLength:last_col(), \(x) max(x, na.rm = TRUE))) 
#now the diet by number file that has the environmentals

env = read_csv("data/shipchannel_dietbynumb.csv") %>%
  select(SampleID, Date, SurfaceTemperature, SurfaceConductivity, SurfacePPT, Secchi, Turbidity, latitude, longitude) %>%
  group_by(SampleID) %>%
  mutate(serial = lastgroup(SampleID), SampleID2 = str_remove(SampleID, serial),
         serial = str_remove(serial, "^0+")) 

#OK, do sample ID's match?

foo = filter(wide2.1, !SampleID2 %in% env$SampleID2)

foo2 = filter(env, !SampleID2 %in% wide2.1$SampleID2)


#uh... so we're done here? It looks like we're just misisng a few samples where all they ate was amphipods. 

wide3 = left_join(wide2.1, env, by = c("SampleID2", "serial"))

test = filter(wide3, is.na(SurfaceTemperature))
...
wide3 = mutate(wide3, DOY = yday(Date), daylength = daylength(latitude, DOY))

write.csv(wide3, "data/diets_with_environmentals.csv")

#################################################################
#add environmentals to the version with mass
wide2 = read_csv("diet_ibmr_mass.csv")
wide2x = wide2
...
write.csv(wide3, "data/diets_with_enviro_mass.csv")
