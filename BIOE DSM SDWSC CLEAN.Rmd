...
#  UG <- -0.299
  ...
  Bioenergetics <- function(i, W0, food){  
    ...
    ifelse(TheData$Temp[i] > RTL, 
           VEL <- RK1 * W0^RK4, 
           VEL <- ACT * (W0^RK4) * exp(BACT * TheData$Temp[i])) #with BACT set at zero, this is zero?
    ft.metabolism <- exp(RQ * TheData$Temp[i])
   # ACTIVITY <- exp(RTO*VEL)                           

    #OK, I dont see the "activity" term in Will's code. 
    ...
    Growth <- ((((Consumption) * 
                  ...
                  ((Egestion + Excretion + SDAction) *
                     sum(prey.ed * food_prop) +
                     #I don't see OxyConv anywhere in AWill's code
